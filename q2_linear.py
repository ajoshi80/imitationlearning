...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.r = tf.placeholder(dtype=tf.float32, shape=[None],
                                name='rewards')
        self.sp = tf.placeholder(dtype=tf.float32, shape=[None, img_height, img_width, nchannels*self.config.state_history],
                                name='next_state')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse = reuse):
            ...
            out = tf.layers.dense(flat, num_actions, reuse = reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_net = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = q_scope)
        target_q_net = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = target_q_scope)
        target_q_net = sorted(target_q_net, key=lambda param: param.name)
        q_net = sorted(q_net, key=lambda param: param.name)
        op = [tf.assign(target_q_net[i], q_net[i]) for i in range(len(q_net))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        done = tf.cast(self.done_mask, tf.float32) - 1
        tmp = self.config.gamma * tf.reduce_max(target_q, axis = 1)
        q_samp = self.r - done * tmp
        action = tf.one_hot(self.a, num_actions)
        q_pred = tf.reduce_sum(action * q, axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        scope_parameters = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope)
        grads_vars = optimizer.compute_gradients(self.loss, scope_parameters)
        if self.config.grad_clip:
            clipped_grads_vars = [(tf.clip_by_norm(gv[0], self.config.clip_val), gv[1]) for gv in grads_vars if gv[0] is not None]
        self.train_op = optimizer.apply_gradients(clipped_grads_vars)
        self.grad_norm = tf.global_norm([gv[0] for gv in grads_vars if gv[0] is not None])
