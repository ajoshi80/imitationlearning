...
class Network:
    def __init__(self, hidden_layer_size, learning_rate, checkpoints_dir):
        ...
        self.resnet = ResnetModel(18, 1, 64, 7, 2, 3, 2, [3,4,6,3], [2,2,2,2])

        h = self.resnet(self.observations, True)
    ...
    def forward_pass(self, observations):
        up_probability = self.sess.run(
            self.up_probability,
            feed_dict={self.observations: observations.reshape([1, 210,160,3])})
